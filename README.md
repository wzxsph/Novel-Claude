...
- **多端全覆盖 (GUI & CLI)**: 拥有基于 `customtkinter` 极其美观和现代化的桌面控制台，包含工作流、**插件管理**、配置面板、Prompt 自定义、批量生成监控，并附带直接对接 `EventBus` 的内部终端模块。
...
# 热重载单个指定的插件
uv run python cli.py skills reload core_memory_rag
